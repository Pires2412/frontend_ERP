...
async function editItem(element) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error("Falha de requisição");
        }
...
async function deleteRegistry(id) {
    ...
    let url = `https://backend-application-t80d.onrender.com/products/deleteProduct/${id}`;
    ...
    try {
        ...
        await writinTable(dataSource);

    } catch (xcpt) {

        console.log(xcpt.message);
    }
}
...
function removeItem(element) {
    ...
    modal.classList.add('deleteModal--active');

    let modalDeleteButton = document.querySelector('.removeProduct');

    let modalCloseButton = document.querySelector('.closeModal');

    modalCloseButton.onclick = () => {

        modal.classList.remove('deleteModal--active');
    };
